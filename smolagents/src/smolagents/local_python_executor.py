...
def _configure_matplotlib_backend():
    """Configure matplotlib to use a non-GUI backend to avoid threading issues."""
    try:
        import matplotlib
        matplotlib.use('Agg')  # Use non-GUI backend
        # Suppress the warning about GUI backend in thread
        import warnings
    ...
    if isinstance(expression, ast.Import):
        for alias in expression.names:
            if check_import_authorized(alias.name, authorized_imports):
                # Configure matplotlib backend before importing matplotlib-related modules
                if alias.name in ['matplotlib', 'matplotlib.pyplot', 'pyplot'] or alias.name.startswith('matplotlib.'):
                    _configure_matplotlib_backend()
                raw_module = import_module(alias.name)
                state[alias.asname or alias.name] = get_safe_module(raw_module, authorized_imports)
            else:
                raise InterpreterError(
                    f"Import of {alias.name} is not allowed. Please try to use other libraries."
                )
        return None
    elif isinstance(expression, ast.ImportFrom):
        if check_import_authorized(expression.module, authorized_imports):
            # Configure matplotlib backend before importing matplotlib-related modules
            if expression.module and (expression.module in ['matplotlib', 'matplotlib.pyplot', 'pyplot'] or expression.module.startswith('matplotlib.')):
                _configure_matplotlib_backend()
            ...
            raise InterpreterError(
                f"Import from {expression.module} is not allowed. Please try to use other libraries."
            )
        return None
...
class LocalPythonExecutor(PythonExecutor):
    ...
    def __init__(
        ...
        # Configure matplotlib to use non-GUI backend to prevent threading issues
        _configure_matplotlib_backend()
